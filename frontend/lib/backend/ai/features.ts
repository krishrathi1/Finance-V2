...
// ===========================================================================
// FEATURE 3 — Research report markdown
// ===========================================================================

const REPORT_FALLBACK_TITLE_SUFFIX = " Research Report";
...
export async function researchReport(
  ...
  const ctx = asObj(context);
  const title = `${symbol}${REPORT_FALLBACK_TITLE_SUFFIX}`;

  if (isGeminiConfigured()) {
    const raw = await generateText(buildReportPrompt(symbol, ctx));
    if (raw) {
      return {
        title,
        report: raw,
...
function deriveRiskLevel(context: AnyObj): string {
  const risk = asObj(context.riskScore);
  const score = toFloatDefault(risk.score, 0.0);
  return score < 2 ? "low" : score < 3.5 ? "medium" : "high";
}

function buildReportPrompt(symbol: string, context: AnyObj): string {
  const compactContext = JSON.stringify({
    companyName: context.companyName ?? null,
    symbol: context.symbol ?? null,
    sector: context.sector ?? null,
    ...
    riskScore: context.riskScore ?? null,
    smartScore: context.smartScore ?? null,
    news: asArr(context.news).slice(0, 8),
    financials: context.financials ?? null,
    shareholding: context.shareholding ?? null,
  });
  return (
