...
describe("cache (getFresh/getStale/setCache)", () => {
  ...
  it("goes stale (fresh miss) once the TTL passes, but is still available via getStale", async () => {
    const { getFresh, getStale, setCache } = await import("@/lib/backend/cache");
