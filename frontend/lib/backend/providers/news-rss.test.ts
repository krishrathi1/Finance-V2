...
import { scoreSentiment } from "@/lib/backend/providers/news-rss";

const getTextMock = vi.hoisted(() => vi.fn());
vi.mock("@/lib/backend/http", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/backend/http")>();
...
describe("getGoogleNews", () => {
  ...
  it("parses CDATA titles without corrupting embedded angle-bracket text", async () => {
    ...
    const { getGoogleNews } = await import("@/lib/backend/providers/news-rss");
