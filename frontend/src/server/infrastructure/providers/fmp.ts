 ...
 *
 * Implements {@link FmpProviderApi} from "@/shared/contracts". FMP is used
...
import { getJson, round2, toFloat } from "@/server/infrastructure/http";
import type {
  Candle,
  FmpProviderApi,
  ProviderBundle,
...
async function getFmpQuote(
  marketSymbol: string,
  signal?: AbortSignal,
): Promise<import("@/shared/contracts").RawQuote | null> {
  try {
    const key = apiKey();
    if (!key) return null;
